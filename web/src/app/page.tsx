...
export default function LandingPage() {
  return (
    <div className="w-full h-screen flex flex-col md:flex-row bg-black overflow-hidden font-sans">
      ...
      {/* CENTER: Trench Run (Military) */}
      <Link href="/trench-run" className="group relative flex-1 h-1/3 md:h-full border-b md:border-b-0 md:border-r border-slate-800 hover:flex-[1.2] transition-all duration-500 ease-out overflow-hidden">
        {/* Background */}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 z-10">
          ...
          >
            <span className="text-amber-500 tracking-[0.3em] text-xs font-bold uppercase mb-3 block">Targeting Computer</span>
            <h2 className="text-3xl md:text-5xl font-black text-white mb-2 group-hover:scale-105 transition-transform duration-300 font-mono tracking-tighter">TRENCH RUN</h2>
            <p className="text-neutral-500 max-w-xs mx-auto mt-3 text-sm leading-relaxed font-mono hidden md:block">
              Retro UI Generator. Vector targeting systems.
            </p>
            <div className="mt-6 px-5 py-2 border border-amber-500/30 text-amber-500 rounded text-xs font-bold bg-amber-500/10 group-hover:bg-amber-500 group-hover:text-black transition-all font-mono">
              INITIATE SEQUENCE
            </div>
          </motion.div>
        </div>
      ...
      {/* RIGHT: ezzackly's studio (Fiber Arts) */}
      <Link href="/ezzackly" className="group relative flex-1 h-1/3 md:h-full hover:flex-[1.2] transition-all duration-500 ease-out overflow-hidden">
        {/* Background */}
